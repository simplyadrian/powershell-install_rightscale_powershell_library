...
function New-Zone()
{
	...
	$httpResponse = $route53Client.CreateHostedZone("$domainName","$comment")
...
function Update-ARecord()
{
	...
	#Find Resource Record from domain name
	"Searching for $domainName. in hostedzone: $hostedZoneId "
	
	$route53Client = New-Object Route53Helper.Route53Client
	$httpResponse = $route53Client.ListResourceRecordSets($hostedZoneId)
	$httpContent = [Route53Helper.Route53Client]::GetResponseContentXml($httpResponse)
	$resourceRecord = $httpContent.ListResourceRecordSetsResponse.ResourceRecordSets.ResourceRecordSet | where {$_.Name -eq "$domainName."}
	
	if ($resourceRecord -eq $null)
	{
	    "Error, could not find $domainName. in hostedzone:$hostedZoneId, exitting..."
	    Exit 1
	}
	
	$currentTTL = $resourceRecord.ttl
	$currentIP = $resourceRecord.ResourceRecords.ResourceRecord.value

	
	Remove-ARecord -domainName $domainName -type "A" -ttl "$ttl" -value "$currentIP" -hostedZoneId "$hostedZoneId" -comment "$comment"	
	New-ARecord -domainName $domainName -type "A" -ttl "$currentTTL" -value "$newIP" -hostedZoneId "$hostedZoneId" -comment "$comment"
}
